...
try {
    $pdo = getPDO();
    $items = $pdo->query("SELECT * FROM contratantes ORDER BY nome ASC")->fetchAll();
} catch (PDOException $e) {
    $items = [];
}

renderHead('Contratantes');
renderSidebar('contratantes');
?>
<div class="main-content">
    <div class="page-header">
        <div>
            <h1><i class="bi bi-building me-2" style="color:#a78bfa"></i>Contratantes</h1>
            <div style="color:#6b7280; font-size:0.85rem; margin-top:0.25rem;">Gerenciar contratantes dos fretes</div>
        </div>
        <button class="btn btn-accent" onclick="openModal()">
            <i class="bi bi-plus-lg me-1"></i>Novo Contratante
        </button>
    </div>
    ...
    <div class="table-card">
        ...
        <div class="table-responsive">
            <table class="table" id="mainTable">
                ...
                <tbody>
                    <?php if (empty($items)): ?>
                    <tr><td colspan="8" class="text-center text-muted py-4">Nenhum contratante cadastrado</td></tr>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Novo Contratante</h5>
...
function openModal() {
    document.getElementById('modalTitle').textContent = 'Novo Contratante';
...
function editItem(data) {
    document.getElementById('modalTitle').textContent = 'Editar Contratante';
    document.getElementById('itemId').value = data.id;
    document.getElementById('f_nome').value = data.nome || '';
    document.getElementById('f_tipo').value = data.tipo || 'PJ';
...
function deleteItem(id, nome) {
    ...
    fetch('api.php', { method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({action:'delete', entity:'contratantes', id}) })
        .then(r => r.json()).then(res => {
